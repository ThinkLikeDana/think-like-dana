setLoading(true);" ? { audience, tone, output_format: format } : {}
})"use client";
...
<input type="radio" checked={mode === "BASIC"} onChange={() => setMode("BASIC")} /> BASIC
</label>
<label style={{ marginLeft: 10 }}>
<input type="radio" checked={mode === "DETAIL"} onChange={() => setMode("DETAIL")} /> DETAIL
</label>
</div>
...
<input placeholder="Output format" value={format} onChange={e => setFormat(e.target.value)} />
</div>
)}

<button onClick={optimize} disabled={loading} style={{ marginTop: 10 }}>
